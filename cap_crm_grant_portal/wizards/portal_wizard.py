...
class CRMPortalWizard(models.TransientModel):
    """
        A wizard to manage the creation/removal of portal users.
    """

    _name = 'crm.portal.wizard'
    _description = 'CRM Grant Portal Access'

    def _default_user_ids(self):
        # for each partner, determine corresponding crm.portal.wizard.user records
        crm_ids = self.env['crm.lead'].search([
            ('id', 'in', self.env.context.get('active_ids', []))
        ])
        partner_ids = [x.partner_id for x in crm_ids]
        if not partner_ids:
            raise UserError(_("No Customer Available"))
        contact_ids = set()
        user_changes = []
    ...
    user_ids = fields.One2many('crm.portal.wizard.user', 'crm_wizard_id', string='Users',default=_default_user_ids)
...
class PortalWizardUser(models.TransientModel):
    ...
    _name = 'crm.portal.wizard.user'
    _description = 'CRM Portal User Config'

    crm_wizard_id = fields.Many2one('crm.portal.wizard', string='Wizard', required=True, ondelete='cascade')
    ...
    def _send_email(self):
        ...
        template = self.env.ref('cap_crm_grant_portal.mail_template_data_crm_portal_welcome')
